...
class Network():
    ...
    def __init__(self):
        ...
        with self.graph.as_default():
            ...
            self.masks = tf.placeholder('float', [None, self.n_input])
            ...
            self.masked_X = tf.multiply(self.X, self.masks)

            self.enc = self.encoder(self.masked_X)
            self.dec = self.decoder(self.enc)



            self.output = self.dec
            self.diff = tf.multiply(self.output - self.X, self.masks)
            self.loss = tf.reduce_mean(tf.pow(self.diff, 2))
            self.true_loss = tf.reduce_mean(tf.pow(self.output - self.X, 2))
    ...
    def save(self):
        with self.graph.as_default():
            with self.sess.as_default():
                ...
                save_path = saver.save(self.sess, "./tmp/sparse_model.ckpt")
    ...
    def load(self):
        with self.graph.as_default():
            ...
            with self.sess.as_default():
                saver = tf.train.Saver()
                saver.restore(self.sess, "./tmp/sparse_model.ckpt")
    ...
    def eval(self, fd):
        with self.graph.as_default():
            with self.sess.as_default():
                return self.sess.run(self.true_loss, fd)
